// ─── SUBJECT ADVISOR EVALUATE PIPELINE ────────────────────────────────────────
// Orchestrates the full evaluation flow:
//   Input → Prompt → Groq LLM → Parse → Sanitize → Respond (or Fallback)
//
// This module contains NO HTTP/NextResponse logic — it's a pure service.
...
// ─── SMART FALLBACK ────────────────────────────────────────────────────────────
// Builds a deterministic fallback when Groq is unavailable or returns garbage.
...
function buildFallback(classLevel, answers) {
  ...
  if (classLevel === "10th") {
    ...
    return {
      ...
      secondary_options,
      dominant_traits,
      confidence_score: AI_CONFIG.scoring.fallbackConfidence,
...
function processClass10Response(parsed) {
  ...
  return {
    ...
    confidence_level: parsed.confidence_level || getConfidenceLevel(parsed.confidence_score),
...
/**
 * Runs the full subject advisor evaluation pipeline.
 * @param {{ answers: Array, classLevel: string }} input
 * @returns {{ result: object }} Evaluation result
 */
export async function runEvaluatePipeline({ answers = [], classLevel = "12th" }) {
  const groqApiKey = process.env.GROQ_API_KEY;

  if (!groqApiKey) {
    logger.warn(TAG, "GROQ_API_KEY not configured — using fallback");
    return { result: buildFallback(classLevel, answers) };
  }
  ...
  // ─── CALL GROQ LLM ─────────────────────────────────────────────────
  logger.pipeline("evaluate", `Sending Class ${classLevel} answers to Groq`);

  const { groq } = AI_CONFIG;
  const groqResponse = await safeFetch(groq.apiUrl, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      temperature: groq.defaults.temperature,
      max_tokens: groq.defaults.maxTokens,
      response_format: { type: "json_object" },
    }),
  }, AI_CONFIG.pipelines.fetchTimeoutMs);

  if (!groqResponse || !groqResponse.ok) {
    const errText = groqResponse ? await groqResponse.text().catch(() => "unreadable") : "Network failure";
    logger.error(TAG, "Groq API failed", errText);
    return { result: buildFallback(classLevel, answers) };
  }
  ...
  if (!responseText) {
    logger.error(TAG, "Empty response from Groq");
    return { result: buildFallback(classLevel, answers) };
  }

  logger.pipeline("evaluate", "Groq response received, parsing...");
  ...
  if (classLevel === "10th") {
    ...
    if (result) {
      logger.info(TAG, "Class 10 analysis complete", { streams: result.recommended_streams });
      return { result };
    }
  } else {
    const result = processClass12Response(parsed);
    if (result) {
      logger.info(TAG, "Class 12 analysis complete", { path: result.recommended_path });
      return { result };
    }
  }

  logger.warn(TAG, "Parsed result missing expected fields, using fallback");
  return { result: buildFallback(classLevel, answers) };
}
